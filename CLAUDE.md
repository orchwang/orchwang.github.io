...
│   │   └── Fintech/                # categories: [Articles, Fintech]
...
| `Fintech` | 결제망·은행 시스템·금융 기술 및 금융 인프라의 구조와 운영 |
...
- **Articles** (nested: AI-Engineering, AI-Industry, AI-Essays, Security, Engineering-Culture, Career-Life, Systems-Programming, ML-Theory, Fintech): external article analyses (one post per article, via `article-manager`)
...
- Articles category + `article-manager` subagent: one analysis/intro post per external article (51 posts so far — AI/engineering articles plus YouTube video summaries via yt-dlp transcript extraction), now organized into 9 nested sub-categories (AI-Engineering, AI-Industry, AI-Essays, Security, Engineering-Culture, Career-Life, Systems-Programming, ML-Theory, Fintech); the subagent recommends a new sub-category when an article fits none
