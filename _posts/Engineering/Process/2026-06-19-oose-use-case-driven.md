...
<svg role="img" aria-label="하나의 유스케이스가 개발 5단계를 관통하는 그림. '주문 결제'라는 유스케이스가 척추처럼 가로로 길게 뻗어, 요구사항·분석·설계·구현·테스트 다섯 단계를 차례로 꿰뚫는다. 각 단계 위에는 그 유스케이스가 남긴 산출물(유스케이스 기술서, 경계·제어·엔티티 객체, 구체 클래스, 코드, 시나리오별 테스트)이 놓여 있고, 모든 산출물은 같은 유스케이스 식별자로 추적된다." viewBox="0 0 680 300" xmlns="http://www.w3.org/2000/svg">
  <title>유스케이스 주도 개발 — 하나의 유스케이스가 요구·분석·설계·구현·테스트를 관통하는 추적성의 척추</title>

  <!-- top label -->
  <text x="340" y="26" text-anchor="middle" font-size="12" fill="currentColor" font-weight="700" opacity="0.75">하나의 유스케이스가 개발 전 단계를 관통한다</text>

  <!-- the use-case spine: one long bar running through every stage -->
  <rect x="40" y="138" width="600" height="24" rx="12" fill="var(--bg-light)" stroke="var(--accent-color)" stroke-width="2.5"/>
  <text x="62" y="155" text-anchor="start" font-size="10.5" fill="currentColor" font-weight="700">유스케이스 «주문 결제»</text>
  <!-- spine arrowhead pointing right -->
  <path d="M640,150 L660,150" stroke="var(--accent-color)" stroke-width="2.5" marker-end="url(#uc-spine)"/>

  <!-- five stage stems rising from the spine to their artefact boxes -->
  <g stroke="var(--secondary-color)" stroke-width="2">
    <line x1="100" y1="138" x2="100" y2="112" marker-end="url(#uc-arrow)"/>
    <line x1="230" y1="138" x2="230" y2="112" marker-end="url(#uc-arrow)"/>
    <line x1="360" y1="138" x2="360" y2="112" marker-end="url(#uc-arrow)"/>
    <line x1="490" y1="138" x2="490" y2="112" marker-end="url(#uc-arrow)"/>
    <line x1="600" y1="162" x2="600" y2="200" marker-end="url(#uc-arrow)"/>
  </g>

  <!-- stage 1: 요구사항 (artefact above) -->
  <rect x="52" y="62" width="96" height="48" rx="4" fill="var(--bg-panel)" stroke="currentColor" stroke-width="1.8"/>
  <text x="100" y="82" text-anchor="middle" font-size="10.5" fill="currentColor" font-weight="700">요구사항</text>
  <text x="100" y="98" text-anchor="middle" font-size="8" fill="currentColor" opacity="0.8">유스케이스 기술서</text>

  <!-- stage 2: 분석 -->
  <rect x="178" y="62" width="104" height="48" rx="4" fill="var(--bg-panel)" stroke="currentColor" stroke-width="1.8"/>
  <text x="230" y="82" text-anchor="middle" font-size="10.5" fill="currentColor" font-weight="700">분석</text>
  <text x="230" y="98" text-anchor="middle" font-size="8" fill="currentColor" opacity="0.8">B·C·E 객체</text>

  <!-- stage 3: 설계 -->
  <rect x="308" y="62" width="104" height="48" rx="4" fill="var(--bg-panel)" stroke="currentColor" stroke-width="1.8"/>
  <text x="360" y="82" text-anchor="middle" font-size="10.5" fill="currentColor" font-weight="700">설계</text>
  <text x="360" y="98" text-anchor="middle" font-size="8" fill="currentColor" opacity="0.8">구체 클래스</text>

  <!-- stage 4: 구현 -->
  <rect x="440" y="62" width="100" height="48" rx="4" fill="var(--bg-panel)" stroke="currentColor" stroke-width="1.8"/>
  <text x="490" y="82" text-anchor="middle" font-size="10.5" fill="currentColor" font-weight="700">구현</text>
  <text x="490" y="98" text-anchor="middle" font-size="8" fill="currentColor" opacity="0.8">코드</text>

  <!-- stage 5: 테스트 (artefact below — closes the chain, highlighted in gold) -->
  <rect x="546" y="202" width="108" height="48" rx="4" fill="var(--bg-panel)" stroke="var(--gold)" stroke-width="2.5"/>
  <text x="600" y="222" text-anchor="middle" font-size="10.5" fill="currentColor" font-weight="700">테스트</text>
  <text x="600" y="238" text-anchor="middle" font-size="8" fill="currentColor" opacity="0.8">시나리오별 케이스</text>

  <!-- traceability note -->
  <text x="300" y="284" text-anchor="middle" font-size="9.5" fill="currentColor" opacity="0.8" font-weight="700">같은 식별자로 끝까지 추적 — 요구가 바뀌면 변경 경로가 한눈에 보인다</text>

  <defs>
    <marker id="uc-arrow" markerWidth="8" markerHeight="8" refX="6" refY="4" orient="auto">
      <path d="M0,0 L8,4 L0,8 z" fill="var(--secondary-color)"/>
    </marker>
    <marker id="uc-spine" markerWidth="9" markerHeight="9" refX="7" refY="4.5" orient="auto">
      <path d="M0,0 L9,4.5 L0,9 z" fill="var(--accent-color)"/>
    </marker>
  </defs>
</svg>
<figcaption>이 글의 척추 — 하나의 유스케이스 «주문 결제»가 <strong>요구사항 → 분석 → 설계 → 구현 → 테스트</strong> 다섯 단계를 관통한다. 각 단계가 남긴 산출물(유스케이스 기술서 · B·C·E 객체 · 구체 클래스 · 코드 · 시나리오별 테스트)은 모두 같은 유스케이스 식별자로 추적되어, 요구가 바뀌면 어디를 고쳐야 할지가 한눈에 드러난다.</figcaption>
</figure>
...
세 책임 유형이 서로 **다른 이유로** 바뀐다는 것을 한 장으로 보면 다음과 같습니다.

<figure class="post-figure">
<svg role="img" aria-label="경계·제어·엔티티 세 객체가 각자 다른 변화의 축에 대응하는 그림. 왼쪽에 세 가지 변화 요인(UI 변경, 업무 절차 변경, 도메인 규칙 변경)이 있고, 각 요인이 각각 Boundary, Control, Entity 하나에만 화살표로 연결된다. 화살표는 서로 교차하지 않아, 한 종류의 변화가 다른 책임 유형으로 번지지 않음을 보여준다. 오른쪽에는 세 객체가 위에서 아래로 변동성이 큰 순서(Boundary는 자주, Control은 중간, Entity는 거의 안 변함)로 놓인다." viewBox="0 0 640 280" xmlns="http://www.w3.org/2000/svg">
  <title>B·C·E의 변화의 축 — UI 변경은 Boundary로, 업무 절차 변경은 Control로, 도메인 규칙 변경은 Entity로만 전달된다</title>

  <!-- column headers -->
  <text x="120" y="28" text-anchor="middle" font-size="11.5" fill="currentColor" font-weight="700" opacity="0.75">변화 요인</text>
  <text x="500" y="28" text-anchor="middle" font-size="11.5" fill="currentColor" font-weight="700" opacity="0.75">흔들리는 책임</text>

  <!-- left: change drivers -->
  <rect x="36" y="56" width="168" height="40" rx="4" fill="var(--bg-light)" stroke="currentColor" stroke-width="1.8"/>
  <text x="120" y="81" text-anchor="middle" font-size="10.5" fill="currentColor" font-weight="700">UI·접점 변경</text>
  <rect x="36" y="120" width="168" height="40" rx="4" fill="var(--bg-light)" stroke="currentColor" stroke-width="1.8"/>
  <text x="120" y="145" text-anchor="middle" font-size="10.5" fill="currentColor" font-weight="700">업무 절차 변경</text>
  <rect x="36" y="184" width="168" height="40" rx="4" fill="var(--bg-light)" stroke="currentColor" stroke-width="1.8"/>
  <text x="120" y="209" text-anchor="middle" font-size="10.5" fill="currentColor" font-weight="700">도메인 규칙 변경</text>

  <!-- non-crossing arrows: each driver hits exactly one stereotype -->
  <g stroke="var(--accent-color)" stroke-width="2.2" fill="none">
    <line x1="204" y1="76" x2="412" y2="76" marker-end="url(#bce-arrow)"/>
    <line x1="204" y1="140" x2="412" y2="140" marker-end="url(#bce-arrow)"/>
    <line x1="204" y1="204" x2="412" y2="204" marker-end="url(#bce-arrow)"/>
  </g>

  <!-- right: stereotypes, ordered by volatility (top = most volatile) -->
  <rect x="416" y="56" width="188" height="40" rx="4" fill="var(--bg-panel)" stroke="var(--accent-color)" stroke-width="2.2"/>
  <text x="510" y="74" text-anchor="middle" font-size="10.5" fill="currentColor" font-weight="700">Boundary «경계»</text>
  <text x="510" y="89" text-anchor="middle" font-size="8" fill="currentColor" opacity="0.8">자주 변함 — 화면·API</text>
  <rect x="416" y="120" width="188" height="40" rx="4" fill="var(--bg-panel)" stroke="var(--secondary-color)" stroke-width="2.2"/>
  <text x="510" y="138" text-anchor="middle" font-size="10.5" fill="currentColor" font-weight="700">Control «제어»</text>
  <text x="510" y="153" text-anchor="middle" font-size="8" fill="currentColor" opacity="0.8">가끔 변함 — 흐름 조율</text>
  <rect x="416" y="184" width="188" height="40" rx="4" fill="var(--bg-panel)" stroke="var(--gold)" stroke-width="2.2"/>
  <text x="510" y="202" text-anchor="middle" font-size="10.5" fill="currentColor" font-weight="700">Entity «엔티티»</text>
  <text x="510" y="217" text-anchor="middle" font-size="8" fill="currentColor" opacity="0.8">거의 안 변함 — 도메인 규칙</text>

  <text x="320" y="258" text-anchor="middle" font-size="9.5" fill="currentColor" opacity="0.8" font-weight="700">화살표가 교차하지 않는다 — 한 종류의 변화가 다른 책임으로 번지지 않는다</text>

  <defs>
    <marker id="bce-arrow" markerWidth="8" markerHeight="8" refX="6" refY="4" orient="auto">
      <path d="M0,0 L8,4 L0,8 z" fill="var(--accent-color)"/>
    </marker>
  </defs>
</svg>
<figcaption>B·C·E가 책임을 가르는 진짜 이유 — 세 객체는 <strong>서로 다른 축</strong>에서 변한다. UI가 바뀌면 Boundary만, 업무 절차가 바뀌면 Control만, 도메인 규칙이 바뀌면 Entity만 흔들린다. 화살표가 교차하지 않으므로 한 종류의 변화가 다른 유형으로 번지지 않는다.</figcaption>
</figure>
