...
published: true
banner: wartable
excerpt: "소프트웨어 아키텍처를 4권의 고전과 1권의 실전 핸드북으로 정복하는 마스터 로드맵입니다. 도메인 모델링부터 품질 속성, 분산 시스템, 아키텍트의 역할, 팀과 함께 설계하는 실전까지 5단계 도장깨기 방식으로 진행합니다."
---
...
<figcaption>4권의 고전이 한 단씩 오르는 아키텍처 학습 여정 — <strong>기초</strong>는 도메인을 중심에 둔 모델링(1 · DDD), <strong>핵심</strong>은 품질 속성을 떠받치는 전술(2 · SAIP)과 복제·분산된 데이터 시스템(3 · DDIA), <strong>응용</strong>은 기계실과 임원실을 오르내리는 아키텍트의 역할(4 · Elevator)이다. 그 위에 <strong>실전</strong> 5단계 *Design It!* 이 팀 워크숍으로 이어진다.</figcaption>
</figure>
...
이 시리즈는 검증된 4권의 고전 위에, 팀과 함께 설계하는 실전 핸드북을 얹습니다. 도메인 중심 사고를 정립하는 Eric Evans의 *Domain-Driven Design*, 아키텍처를 학문으로 체계화한 Bass·Clements·Kazman의 *Software Architecture in Practice*, 현대 분산 데이터 시스템의 교과서인 Martin Kleppmann의 *Designing Data-Intensive Applications*, 기술과 조직을 잇는 아키텍트의 역할을 다룬 Gregor Hohpe의 *The Software Architect Elevator*, 그리고 화이트보드 앞에서 팀과 함께 설계하는 법을 훈련하는 Michael Keeling의 *Design It!* 입니다. 각 책이 하나의 단계가 되어, 기초 → 핵심 → 응용 → 실전으로 이어집니다.
...
5단계는 아래 순서대로 진행하는 것을 권장합니다. **기초**(도메인 중심 사고)로 문제 공간을 보는 눈을 기르고, **핵심**(아키텍처 이론·분산 시스템 실전)으로 설계 역량을 정복한 뒤, **응용**(아키텍트의 역할·소프트스킬)으로 기술과 조직을 잇고, **실전**(팀 디자인 스튜디오)으로 그 모든 것을 워크숍에서 돌립니다.
...
flowchart TD
    ...
    subgraph PRACTICE["실전 · 팀과 함께 설계하기"]
        S5["5단계<br/>Design It!<br/>디자인 싱킹 · 38 활동"]
    end
...
## 5단계: Design It! — 팀과 함께 설계하는 실전

Michael Keeling의 *Design It!: From Programmer to Software Architect* (*개발자에서 아키텍트로*)는 아키텍처를 돌판의 청사진이 아니라 **팀과 함께 돌리는 순환**으로 다룹니다. 디자인 싱킹과 위험 주도 설계, ASR, 38가지 팀 활동이 화이트보드 앞의 실전 훈련이 됩니다.

- [x] **아키텍트의 여섯 가지 책임**: 문제 정의·분리와 위임·큰 그림·트레이드오프·기술 부채·팀의 설계 역량 — [[상세](/2026/09/06/design-it-from-programmer-to-architect.html)]
- [x] **디자인 싱킹 (HART · 네 마인드셋)**: 인간중심·모호함·재디자인·촉각, 이해하기·탐색하기·실현하기·평가하기 — [[상세](/2026/09/06/design-it-from-programmer-to-architect.html)]
- [x] **위험 주도 설계와 설계 최적점**: 만족스러운 설계, 베임의 선행 설계 투자, 위험이 다음 마인드셋을 고른다 — [[상세](/2026/09/06/design-it-from-programmer-to-architect.html)]
- [x] **ASR (Architecturally Significant Requirements)**: 제약·품질 속성 시나리오 6요소·영향력 있는 기능·콘웨이 법칙 — [[상세](/2026/09/06/design-it-from-programmer-to-architect.html)]
- [x] **분기-융합과 의사결정 매트릭스**: 대안을 벌리고 품질로 접으며, 결정은 미룰 수 있을 때까지 미룬다 — [[상세](/2026/09/06/design-it-from-programmer-to-architect.html)]
- [x] **은빛 도구상자 (38가지 팀 활동)**: 문제 이해·해결 탐색·실체화·평가 서랍을 막히는 순간에 연다 — [[상세](/2026/09/06/design-it-from-programmer-to-architect.html)]
...
- **설계는 팀 활동입니다**: 품질 시나리오와 통역 능력도, 화이트보드 앞에서 함께 분기하고 융합할 때 비로소 살아 있는 아키텍처가 됩니다.
- **모든 단계는 서로를 보강합니다**: DDD의 바운디드 컨텍스트는 분산 시스템의 서비스 경계로, 품질 속성은 아키텍트의 의사결정 근거로, 그 근거는 팀 워크숍의 재료로 이어집니다.

다섯 단계는 순서대로 쌓일 뿐 아니라, 한 단계의 개념이 다음 단계의 입력이 되며 서로를 보강합니다. 아래는 그 연결 고리를 한 장으로 옮긴 그림입니다.
...
flowchart LR
    ...
    S2 -->|"QA 시나리오가<br/>워크숍 재료로"| S5
    S4 -->|"통역이<br/>팀 활동으로"| S5
```

## 추천 학습 순서

위에 제시한 1 → 2 → 3 → 4 → 5단계 순서를 권장합니다. 먼저 *Domain-Driven Design* 으로 "무엇을, 왜 만드는가"를 보는 도메인 중심 사고를 기릅니다. 이 토대가 있어야 이후의 설계 결정이 목적을 잃지 않습니다. 다음으로 *Software Architecture in Practice* 로 아키텍처를 품질 속성과 전술이라는 공학적 언어로 다루는 법을 익히고, 그 위에서 *Designing Data-Intensive Applications* 로 분산 데이터 시스템의 실전 트레이드오프를 체득합니다. *The Software Architect Elevator* 로 기술 역량을 조직과 비즈니스에 연결한 뒤, 마지막으로 *Design It!* 으로 그 모든 언어를 팀 워크숍에서 돌리는 실전 근육을 만듭니다. 이론(2단계)과 분산 실전(3단계)은 병행해도 좋지만, 도메인 사고(1단계)를 가장 먼저, 팀 설계 실전(5단계)을 가장 나중에 두는 흐름은 유지하길 권합니다.

## 결론

소프트웨어 아키텍처는 하루아침에 익힐 수 있는 기술이 아니라, 도메인을 읽는 눈·품질을 정의하는 언어·분산 시스템의 트레이드오프·조직을 움직이는 리더십·팀과 함께 설계하는 순환이 겹겹이 쌓여 만들어지는 종합 역량입니다. 이 5권은 그 각각의 층을 단단히 다져 줄 검증된 안내서입니다.
...
- [Design It!: 개발자에서 아키텍트로](/2026/09/06/design-it-from-programmer-to-architect.html) — 5단계 실전 핸드북
