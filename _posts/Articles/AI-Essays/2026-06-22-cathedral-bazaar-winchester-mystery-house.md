...
시장 모델의 동력은 분산된 피드백이었다. Raymond의 유명한 명제, 흔히 **Linus의 법칙**이라 불리는 *"눈이 충분히 많으면 모든 버그는 얕다(Given enough eyeballs, all bugs are shallow)"* 가 그 핵심이다. 이 분산 피드백이 사반세기 동안 오픈소스의 우세를 만들었다.
...
즉 *"커밋당 1,000줄은 인간 프로그래머의 하루 생산량보다 약 두 자릿수 배 높다."* 이 비용 폭락은 인터넷이 시장(bazaar)을 가능케 한 사건과 닮았지만, **피드백의 동역학을 뒤집는다.**

- 시장 모델: 기여자가 많아 피드백의 *처리량*은 높지만, 변경의 *지연*은 길다(이슈 등록 → 토론 → PR 리뷰).
- AI 코딩: *구현*은 기계 속도로 빨라졌는데, *피드백과 조율*은 여전히 인간 속도다.

여기서 원문의 결정적 문장이 나온다. **"AI가 만든 코드의 속도로 움직이는 피드백 소스는 단 하나뿐이다 — 바로 당신 자신이다."** 설계 루프가 개인의 취향과 필요로 붕괴(collapse)한다.
...
원문이 드는 동시대의 '윈체스터 미스터리 하우스'들:

- Steve Yegge의 *"Gas Town"* — 지독하게 개인적이고 끝없이 뻗어 나가는 도구.
- Jeffrey Emanuel의 *"Agent Flywheel"* — SQLite, Node.js, btrfs, Redis, pandas, NumPy, JAX, Torch를 Rust로 다시 쓴 *"FrankenSuite"* 를 포함.
- Gary Tan의 *"gstack"* — 대부분 Markdown으로 조립한 개인용 AI 위원회.
- Philip Zeyliger의 관찰: *"모두가 소프트웨어 공장을 짓고 있다."*
...
- 좋은 신호가 잡음에 묻힌다. 이제 어려운 일은 *소프트웨어에서 버그를 찾는 것*이 아니라, **더 나은 리뷰 프로세스로 버그가 소프트웨어에 도달하지 못하게 막는 것**이다.

구조적 문제의 정수: **"공동의 작업으로 정의되는 시장(bazaar)은 이 해킹을 쓸 수 없다. 시장 속의 코딩 에이전트는 엉망을 만든다 — 기계 속도의 구현이 인간 속도로 지어진 조율 인프라에 부딪힌다."** 개인은 자기 자신이라는 단일 피드백으로 루프를 닫을 수 있지만, 공동체는 그럴 수 없다.

### 미스터리 하우스가 주는 세 가지 교훈

1. **공존할 수 있다.** 시장과 미스터리 하우스는 양립한다(원문은 OpenClaw를 예로 든다).
2. **재미있는 부분은 팔지 마라(Don't sell the fun stuff).** 공통의 핵심(core)과 개인의 커스터마이징을 분리하라. OpenClaw의 전략대로 *"지루하지만 치명적인 것"* 에 협업을 집중하라 — *"지루한 것, 어려운 부분, 혹은 실패의 대가가 재앙적인 것들이야말로 함께 만들어야 할 것이다."* 스테인드글라스(개인 취향)는 사용자에게 맡기고, 배관(plumbing)은 commons로.
3. **코드의 한계는 결국 소통이다(The limits of code are communication).** 기계 속도에서 *주의(attention)를 관리하는 문제*는 여전히 풀리지 않았다.
