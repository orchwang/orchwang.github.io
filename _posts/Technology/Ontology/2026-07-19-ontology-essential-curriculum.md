...
- [ ] **데이터 모델 vs 스키마 vs 온톨로지**: 저장 구조와 의미 계층의 구분, 각각이 답하는 질문
- [ ] **왜 의미 계층인가**: 조인·컬럼만으로 잃어버리는 도메인 의미, 공유 어휘로서의 온톨로지
- [ ] **분석에서 운영으로**: read-only 데이터 모델과 "행동의 시스템"으로서의 온톨로지, 디지털 트윈
...
- [ ] **지식 그래프와 RDF/OWL**: 트리플·클래스·프로퍼티, SPARQL, 시맨틱 웹의 계보
- [ ] **속성 그래프 모델**: 노드·엣지·속성, RDF와의 차이, 그래프 DB에서의 실무 표현
- [ ] **택소노미·기술논리**: 분류 체계와 온톨로지의 차이, 추론(inference)이 가능해지는 지점
...
- [ ] **객체 타입**: 도메인 명사를 객체로 승격하기, 무엇을 객체로/속성으로 둘지의 판단
- [ ] **속성과 타입**: 속성의 데이터 타입, 파생 속성, 표시용 이름과 식별자
- [ ] **기본키와 객체 그래프**: 유일 식별, ER 모델과의 관계, 객체 그래프의 밑그림
...
- [ ] **링크 타입**: 관계를 일급 개념으로, 조인 로직 대신 모델에 새겨진 관계
- [ ] **카디널리티와 외래키 매핑**: 1:1·1:N·N:M, 외래키·조인 테이블 → 링크로의 번역
- [ ] **그래프 탐색**: 링크를 따라 이동하며 답을 얻기, 다대다 관계와 경로 질의
...
- [ ] **백킹 데이터셋과 속성 매핑**: 파이프라인 산출물 → 객체·속성, 컬럼 매핑과 파생
- [ ] **엔티티/식별자 해소**: 여러 소스의 같은 실체를 하나로, 매칭 규칙과 신뢰도
- [ ] **지저분한 현실 다루기**: 중복·불일치·누락, 데이터 품질과 온톨로지의 신뢰성
...
- [ ] **액션과 write-back**: 통제된 객체 변경, 행동의 결과가 모델·원천으로 되돌아오는 고리
- [ ] **함수 기반 액션과 검증**: 임의 로직·규칙, 액션 단위의 검증과 권한
- [ ] **운영과 분석의 통합**: 하나의 의미 계층 위에서 만나는 read/write, 업무 시스템으로서의 온톨로지
...
- [ ] **온톨로지 진화**: 객체·링크·스키마의 버전 관리, 하위 호환과 마이그레이션
- [ ] **거버넌스와 보안**: 의미 계층 위의 접근 제어·보안 마킹, 데이터 계보·감사
- [ ] **FDE 워크플로**: 도메인 워크숍으로 요구 도출, 반복적 모델링, 배포·피드백 루프
