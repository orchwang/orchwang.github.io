...
**[유닛 1 · 기초]** 코스 전체의 지도를 그리는 단계. "밑바닥부터 만든다"는 것이 무엇이고 왜 **효율**이 이 강의의 중심 주제인지 이해한 뒤, 언어 모델의 첫 관문인 **토크나이저**를 다룹니다. 문자·바이트 단위의 한계에서 출발해, 오늘날 거의 모든 LLM이 쓰는 **BPE(Byte Pair Encoding)**를 직접 구현해 봅니다.

- [ ] **코스 개요와 철학**: "from scratch"의 의미, 효율(연산·메모리·데이터)이라는 관통 주제, 3대 빌딩블록(아키텍처·시스템·데이터)
- [ ] **토크나이제이션의 필요성**: 문자/바이트 기반의 한계, 어휘 크기 ↔ 시퀀스 길이 트레이드오프, 서브워드의 등장
- [ ] **BPE(Byte Pair Encoding)**: 병합 규칙 학습, 인코딩·디코딩, GPT-2 토크나이저와 바이트 수준 BPE

## 2단계: PyTorch와 자원 회계 (Lecture 2)

**[유닛 1 · 기초]** 모델을 "느낌"이 아니라 **숫자**로 다루는 법을 익히는 단계. 텐서의 메모리 레이아웃과 연산을 PyTorch로 짚고, 모델 크기·연산량(FLOPs)·메모리를 직접 추정하는 **자원 회계(resource accounting)**를 배웁니다. 이후 모든 시스템·스케일링 논의는 이 회계 위에서 이뤄집니다.

- [ ] **텐서와 메모리**: dtype(fp32/bf16/fp8)·메모리 레이아웃·뷰 vs 복사, 파라미터·활성화·옵티마이저 상태의 메모리
- [ ] **FLOPs 회계**: forward/backward 연산량, 행렬곱이 지배하는 비용, 모델 크기 대비 학습 FLOPs(6ND 근사)
- [ ] **MFU와 학습 비용 추정**: 하드웨어 이론 성능 대비 활용률(MFU), 토큰·파라미터·GPU 시간으로 학습 비용 가늠하기
...
이 시리즈는 이제 막 출발선에 섰습니다(0%). 17단계를 하나씩 정복하며, "API 뒤의 블랙박스"를 "내가 이해하고 만들 수 있는 시스템"으로 바꿔 나가겠습니다. 각 단계를 완주할 때마다 상세 포스트를 연결하고 진행률을 갱신합니다.

### 다음 학습 (Next Learning)

- **1단계: 개요와 토크나이제이션** — [Lecture 1 영상](https://www.youtube.com/watch?v=SQ3fZ1sAqXI&list=PLoROMvodv4rOY23Y0BoGoBGgQ1zmU_MT_)부터 시작 (상세 포스트 작성 예정)
