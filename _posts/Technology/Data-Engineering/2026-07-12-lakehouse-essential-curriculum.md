...
- [x] **파일 레이크의 한계**: 디렉터리·파일 목록 기반 Hive 테이블의 원자성·일관성 문제, listing 비용 — [[상세](/2026/07/15/lakehouse-open-table-format-motivation.html)]
- [x] **테이블 포맷이 여는 것**: 파일 집합 위에 스냅샷·스키마·통계를 얹어 "테이블"로 만들기 — [[상세](/2026/07/15/lakehouse-open-table-format-motivation.html)]
- [x] **레이크하우스와의 관계**: 오버뷰 4단계에서 본 웨어하우스·레이크·레이크하우스 구도에서 테이블 포맷의 자리 — [[상세](/2026/07/15/lakehouse-open-table-format-motivation.html)]
...
- [x] **3계층 구조**: 메타데이터 파일 → 매니페스트 리스트/매니페스트 → 데이터 파일 — [[상세](/2026/07/15/lakehouse-iceberg-metadata-manifests.html)]
- [x] **스냅샷과 매니페스트**: 각 스냅샷이 참조하는 매니페스트 집합, 파일 추가/삭제의 표현 — [[상세](/2026/07/15/lakehouse-iceberg-metadata-manifests.html)]
- [x] **통계와 프루닝**: 파일 수준 통계(min/max·null)로 스캔을 줄이는 플래닝 최적화 — [[상세](/2026/07/15/lakehouse-iceberg-metadata-manifests.html)]
...
- [x] **원자적 커밋과 격리**: 메타데이터 포인터 스왑 기반 optimistic concurrency, 스냅샷 격리 — [[상세](/2026/07/15/lakehouse-iceberg-acid-snapshots-time-travel.html)]
- [x] **시간여행과 롤백**: 스냅샷 ID/타임스탬프 조회, 잘못된 커밋의 되돌리기 — [[상세](/2026/07/15/lakehouse-iceberg-acid-snapshots-time-travel.html)]
- [x] **동시성**: 충돌 감지와 커밋 재시도, 여러 writer가 붙을 때의 동작 — [[상세](/2026/07/15/lakehouse-iceberg-acid-snapshots-time-travel.html)]
...
- [x] **스키마 진화**: 컬럼 ID 기반 추적, add/drop/rename/reorder의 안전성, full/backfill 없는 변경 — [[상세](/2026/07/15/lakehouse-iceberg-partition-schema-evolution.html)]
- [x] **숨은 파티셔닝**: 파티션을 물리 경로가 아닌 메타데이터로 표현, 파티션 변환(transform) — [[상세](/2026/07/15/lakehouse-iceberg-partition-schema-evolution.html)]
- [x] **파티션 진화**: 기존 데이터 재작성 없이 파티션 전략 바꾸기, 진화 전후 파일의 공존 — [[상세](/2026/07/15/lakehouse-iceberg-partition-schema-evolution.html)]
...
- [x] **작은 파일과 compaction**: 작은 파일 문제의 원인과 비용, rewrite/bin-pack·정렬 기반 compaction — [[상세](/2026/07/15/lakehouse-iceberg-compaction-maintenance.html)]
- [x] **스냅샷 만료**: 스냅샷 이력 관리와 만료 정책, 시간여행 보존 기간과의 균형 — [[상세](/2026/07/15/lakehouse-iceberg-compaction-maintenance.html)]
- [x] **파일 정리**: 고아 파일 제거, 매니페스트 리라이트, 정기 유지보수 잡 설계 — [[상세](/2026/07/15/lakehouse-iceberg-compaction-maintenance.html)]
...
- [x] **카탈로그의 역할**: 테이블 식별·커밋 조율, Hive/Glue/JDBC vs REST Catalog — [[상세](/2026/07/15/lakehouse-iceberg-rest-catalog-governance.html)]
- [x] **REST Catalog**: 엔진 중립 표준 인터페이스와 상호운용성, 커밋 프로토콜 — [[상세](/2026/07/15/lakehouse-iceberg-rest-catalog-governance.html)]
- [x] **거버넌스**: 네임스페이스/테이블 접근 제어, credential vending, 감사와 정책 — [[상세](/2026/07/15/lakehouse-iceberg-rest-catalog-governance.html)]
...
- [x] **설계 축으로 비교**: 메타데이터/트랜잭션 모델, 진화, 카탈로그 지원의 포맷별 차이 — [[상세](/2026/07/15/lakehouse-table-format-comparison.html)]
- [x] **워크로드 적합성**: 배치 vs 스트리밍, 조회 중심 vs 업서트 중심에서의 선택 기준 — [[상세](/2026/07/15/lakehouse-table-format-comparison.html)]
- [x] **생태계와 상호운용**: 엔진·클라우드 채택 현황, 포맷 간 호환·전환 접근 — [[상세](/2026/07/15/lakehouse-table-format-comparison.html)]
