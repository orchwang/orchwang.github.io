...
<!-- ============================================================
     ...
     스코프: Spark-Essential 시리즈 7개 스테이지 전체를 훑는 오버뷰 덱.
     대상: Spark를 처음 접하는 초보자. 핵심 용어는 나올 때마다
     deck-note("📖 용어")로 쉬운 한 줄 정의를 곁들이고, 비유로 큰 그림을
     잡게 한다. 흐름: 타이틀 → 왜 Spark → 큰 그림(아키텍처) →
     3막 7단계 순회(용어 풀이 포함) → 핵심 정리 → 완주.
     모든 색은 토큰(var(--…))·currentColor만 — 라이트/다크 양쪽에서 읽힌다.
     한 슬라이드 = 하나의 <section class="slide">.
     ============================================================ -->
<div class="deck-source" hidden aria-hidden="true">

<section class="slide slide--title">
  <p class="deck-kicker">Spark-Essential · 시리즈 한 바퀴</p>
  <h1>Apache Spark<br>7단계로 정복하기</h1>
  <p class="deck-lead">"대용량 데이터를 여러 대가 나눠서 처리한다" — 그 안을 처음부터 들여다봅니다.</p>
  <p class="deck-note">아키텍처부터 레이크하우스까지, 7개 심화편을 하나의 이야기로. 용어는 나올 때마다 쉽게 풀어드립니다.</p>
</section>

<section class="slide">
  <p class="deck-kicker">먼저, 문제부터</p>
  <h2>데이터가 컴퓨터 한 대로<br>감당이 안 된다</h2>
  <p class="deck-lead">수십 GB, 수 TB… 한 대의 메모리·디스크·CPU로는 다 못 담고 못 돈다.</p>
  <ul>
    <li>한 대를 아무리 키워도 <strong>한계</strong>가 있다 (수직 확장의 벽)</li>
    <li>그렇다면 답은 하나 — <strong>여러 대에 나눠서</strong> 처리하자</li>
  </ul>
  <p class="deck-note">그런데 "여러 대로 나눈다"는 건 말처럼 쉽지 않다. 누가 일을 쪼개고, 누가 어디서 처리하고, 한 대가 죽으면 어쩌지? 이걸 대신 해 주는 게 Spark다.</p>
</section>

<section class="slide">
  <p class="deck-kicker">그래서 Spark란</p>
  <h2>나눠서 처리해 주는 엔진</h2>
  <p class="deck-lead">Apache Spark = 큰 데이터를 <strong>조각내 여러 컴퓨터에 뿌리고</strong>, 병렬로 처리해 다시 모아 주는 분산 처리 엔진.</p>
  <ul class="deck-chips">
    <li class="deck-chip">대규모 분산 처리의 사실상 표준</li>
    <li class="deck-chip">배치 + 스트리밍 + ML</li>
    <li class="deck-chip">하나의 엔진으로</li>
  </ul>
  <p class="deck-note">비유하면 — 혼자 하면 며칠 걸릴 설거지를, 접시를 나눠 20명이 동시에 씻고 한곳에 모으는 것. "누구에게 몇 장씩 줄지" 정하는 게 Spark의 핵심이다.</p>
</section>

<section class="slide">
  <p class="deck-kicker">이 발표를 관통하는 그림</p>
  <h2>세우고 · 나눠서 하고 · 다시 모은다</h2>
  <ul class="deck-flow">
    <li>Driver가 계획을 세운다</li>
    <li>Executor들이 나눠서 처리</li>
    <li>셔플로 다시 모은다</li>
    <li>결과</li>
  </ul>
  <p class="deck-lead">Spark의 뼈대는 이 한 줄이다. 뒤의 모든 단계가 이 그림 위에 얹힌다.</p>
  <p class="deck-note">📖 <strong>용어 · 셔플(shuffle)</strong> = 같은 것끼리 모으려고 데이터를 컴퓨터 사이에서 뒤섞어 재배치하는 일. 뒤에서 보겠지만, 늘 가장 비싼(느린) 연산이다.</p>
</section>

<section class="slide">
  <p class="deck-kicker">여정의 지도</p>
  <h2>기반 → 성능 → 활용, 7단계</h2>
  <div class="deck-cols">
    <div class="deck-card">
      <span class="deck-num">기반 1–3</span>
      <p>돌아가게 하고, <strong>왜 빠른지</strong> 이해하기</p>
      <p class="deck-note">아키텍처 · 추상화 · 옵티마이저</p>
    </div>
    <div class="deck-card">
      <span class="deck-num">성능 4</span>
      <p>왜 느려지고 <strong>어떻게 고치나</strong></p>
      <p class="deck-note">셔플 · 파티셔닝 · 튜닝 (실무 가치 최고)</p>
    </div>
    <div class="deck-card">
      <span class="deck-num">활용 5–7</span>
      <p>배치를 넘어 <strong>범위를 넓히기</strong></p>
      <p class="deck-note">스트리밍 · PySpark · 레이크하우스</p>
    </div>
  </div>
</section>

<section class="slide slide--title">
  <p class="deck-kicker">제1막 · 기반</p>
  <h2>돌아가게 만들고,<br>왜 빠른지 이해하기</h2>
  <p class="deck-lead">1단계 아키텍처 · 2단계 추상화 · 3단계 옵티마이저</p>
</section>

<section class="slide">
  <span class="deck-num">1단계 · 아키텍처</span>
  <h2>두뇌 하나, 일꾼 여럿</h2>
  <div class="deck-cols">
    <div class="deck-card">
      <h3>Driver</h3>
      <p>실행 계획을 세우고 일을 나눠 주는 <strong>두뇌</strong>. 클러스터에 딱 하나.</p>
    </div>
    <div class="deck-card">
      <h3>Executor</h3>
      <p>각 컴퓨터에서 실제로 데이터를 처리하는 <strong>일꾼</strong>. 여럿.</p>
    </div>
    <div class="deck-card">
      <h3>클러스터 매니저</h3>
      <p>일꾼을 몇 대·어디에 띄울지 <strong>자원을 배분</strong> (YARN·K8s·Standalone).</p>
    </div>
  </div>
  <p class="deck-note">📖 Executor의 힘은 <strong>core 수</strong>(=동시에 도는 일감 수 = 병렬성)와 <strong>메모리</strong>로 정해진다. 거대한 결과를 두뇌(Driver) 한 대로 끌어오면(collect) 죽는다.</p>
</section>

<section class="slide">
  <span class="deck-num">1단계 · 실행 흐름</span>
  <h2>코드 한 줄이 잘게 쪼개진다</h2>
  <ul class="deck-flow">
    <li>액션</li>
    <li>Job</li>
    <li>Stage</li>
  ...
  <p class="deck-lead">내가 부른 결과 요청(액션) 하나가 Job → Stage → Task로 갈라져 일꾼들에게 뿌려진다.</p>
  <p class="deck-note">📖 <strong>lazy evaluation(지연 실행)</strong> = 변환(filter·groupBy…)은 바로 안 돌고 "계획"만 쌓아 두다가, 결과를 요구하는 <strong>액션</strong>(write·count…)에서 한꺼번에 실행. 덕분에 전체를 보고 더 똑똑하게 최적화한다. 그리고 <strong>스테이지 경계 = 셔플 지점 = 가장 비싼 곳</strong>이다.</p>
</section>

<section class="slide">
  <span class="deck-num">2단계 · 추상화</span>
  <h2>데이터의 세 가지 얼굴</h2>
  <div class="deck-cols">
    <div class="deck-card">
      <h3>RDD</h3>
      <p>1세대 <strong>저수준</strong> 컬렉션. 스키마가 없어 Spark가 안을 못 본다 → 최적화 불가.</p>
    </div>
    <div class="deck-card">
      <h3>DataFrame</h3>
      <p>표처럼 <strong>스키마(컬럼·타입)</strong>가 있는 고수준 API. 옵티마이저가 개입 가능.</p>
    </div>
    <div class="deck-card">
      <h3>Dataset</h3>
      <p>DataFrame + <strong>타입 안전</strong>. 단, JVM(Scala/Java) 전용 — 파이썬엔 없다.</p>
    </div>
  </div>
  <p class="deck-note">📖 <strong>스키마</strong> = "이 컬럼은 금액(숫자), 저 컬럼은 지역(문자)"이라는 구조 정보. PySpark는 사실상 전부 DataFrame을 쓴다.</p>
</section>

<section class="slide">
  <p class="deck-kicker">2단계의 핵심 교훈</p>
  <h2>구조를 알려 주면 대신 빨라진다</h2>
  <p class="deck-lead">DataFrame이 RDD보다 빠른 건 마법이 아니라 <strong>Spark가 데이터 구조를 알아 최적화하기 때문.</strong></p>
  <ul>
    <li><strong>narrow</strong> — 제자리에서 끝나는 변환 (map·filter). 셔플 없음, 싸다.</li>
    <li><strong>wide</strong> — 데이터를 가로질러 모으는 변환 (groupBy·join). <strong>셔플 발생</strong>, 비싸다.</li>
  </ul>
  <p class="deck-note">그래서 실무 원칙 하나 — 특별한 이유가 없으면 <strong>DataFrame/SQL</strong>을 쓴다. 새 코드를 RDD로 시작하면 옵티마이저를 스스로 꺼 버리는 셈.</p>
</section>

<section class="slide">
  <span class="deck-num">3단계 · 옵티마이저</span>
  <h2>왜 빠른가, 세 겹의 엔진</h2>
  <div class="deck-cols">
    <div class="deck-card">
      <h3>Catalyst</h3>
      <p>쿼리를 <strong>더 나은 실행 계획</strong>으로 다시 쓴다. 덜 읽고·덜 옮기게.</p>
    </div>
    <div class="deck-card">
      <h3>Tungsten</h3>
      <p>그 계획을 <strong>하드웨어에 가까운 코드</strong>로 컴파일해 JVM 낭비를 걷어낸다.</p>
    </div>
    <div class="deck-card">
      <h3>AQE</h3>
      <p>실행 <strong>도중</strong> 실제 통계를 보고 계획을 다시 짠다.</p>
    </div>
  </div>
  <p class="deck-note">📖 <strong>옵티마이저</strong> = 내가 시킨 걸 "그대로"가 아니라 "같은 결과를 더 빠르게" 실행하도록 계획을 고쳐 주는 똑똑한 통역사. <strong>AQE</strong> = Adaptive Query Execution(적응형 실행).</p>
</section>

<section class="slide slide--title">
  <p class="deck-kicker">제2막 · 성능</p>
  <h2>왜 느려지고,<br>어떻게 고치나</h2>
  <p class="deck-lead">4단계 셔플 · 파티셔닝 · 튜닝 — 실무 가치가 가장 큰 단계</p>
</section>

<section class="slide">
  <span class="deck-num">4단계 · 튜닝</span>
  <h2>느림의 범인은 거의 셔플</h2>
  <p class="deck-lead">"느리다"는 신고의 대부분은 셋 중 하나 — 셔플 과다, 데이터 쏠림, 잘못된 조인.</p>
  <div class="deck-cols">
    <div class="deck-card">
      <h3>스큐(skew)</h3>
      <p>한 키에 데이터가 <strong>쏠려</strong> 한 일꾼만 지옥. → salting·AQE로 완화.</p>
    </div>
    <div class="deck-card">
      <h3>스필(spill)</h3>
      <p>메모리가 넘쳐 <strong>디스크로 흘러</strong> 급격히 느려짐. → 파티션 잘게.</p>
    </div>
    <div class="deck-card">
      <h3>조인 전략</h3>
      <p>작은 쪽을 방송하는 <strong>broadcast join</strong>이면 셔플을 없앤다.</p>
    </div>
  </div>
  <p class="deck-note">📖 <strong>파티션(partition)</strong> = 나뉜 데이터 한 조각(= 일꾼 하나의 처리 몫). 파티션 수를 일꾼 수에 맞추는 게 튜닝의 기본기. 진단은 <strong>Spark UI</strong>로 한다.</p>
</section>

<section class="slide slide--title">
  <p class="deck-kicker">제3막 · 활용</p>
  <h2>배치를 넘어<br>범위를 넓히기</h2>
  <p class="deck-lead">5단계 스트리밍 · 6단계 PySpark · 7단계 레이크하우스</p>
</section>

<section class="slide">
  <span class="deck-num">5단계 · 스트리밍</span>
  <h2>끝없이 흘러드는 데이터도<br>그냥 테이블처럼</h2>
  <p class="deck-lead">Structured Streaming = 무한 스트림을 <strong>"계속 자라는 테이블"</strong>로 보고, 배치와 거의 같은 코드로 처리.</p>
  <ul>
    <li>새 이벤트 = 테이블에 새 행 추가</li>
    <li>배치에서 익힌 것이 <strong>그대로</strong> 실시간으로 이어진다</li>
  </ul>
  <p class="deck-note">📖 <strong>마이크로배치</strong> = 흐름을 잘게 잘라 작은 배치로 반복 실행. <strong>워터마크</strong> = "언제 창을 닫고 지각 데이터를 버릴지" 정하는 기준선. <strong>상태(state)</strong> = 여러 배치에 걸쳐 기억해야 하는 집계값.</p>
</section>

<section class="slide">
  <span class="deck-num">6단계 · PySpark</span>
  <h2>파이썬으로 쓰지만<br>실행은 JVM에서</h2>
  <p class="deck-lead">현실의 Spark 코드는 대부분 PySpark. 그런데 실행은 여전히 JVM에서 일어난다 — 그 경계에 비용이 있다.</p>
  <ul>
    <li>내장 함수만 쓰면 경계를 안 넘어 <strong>빠르다</strong></li>
    <li>UDF는 편하지만 옵티마이저에게 <strong>블랙박스</strong> — 최후의 수단</li>
  </ul>
  <p class="deck-note">📖 <strong>UDF(사용자 정의 함수)</strong> = 내가 파이썬으로 직접 만든 함수. 느린 Python UDF보다 배치로 처리하는 <strong>Pandas UDF</strong>가 빠르다. 원칙 — "내장 함수로 안 되나?"를 먼저 묻기.</p>
</section>

<section class="slide">
  <span class="deck-num">7단계 · 레이크하우스</span>
  <h2>Spark를 최신 저장 스택에 얹기</h2>
  <p class="deck-lead">Spark는 저장 계층이 없는 <strong>처리 엔진</strong>. Iceberg·Delta 같은 테이블 포맷과 만나면 웨어하우스급이 된다.</p>
  <ul class="deck-chips">
    <li class="deck-chip">ACID 트랜잭션</li>
    <li class="deck-chip">시간여행</li>
    <li class="deck-chip">MERGE·업서트</li>
    <li class="deck-chip">스키마 진화</li>
  </ul>
  <p class="deck-note">📖 <strong>레이크하우스</strong> = 값싼 저장소(S3 등) 위에 창고(웨어하우스)의 능력을 얹은 최신 데이터 스택. <strong>테이블 포맷</strong> = 파일 더미에 "이 스냅샷이 지금 버전"이라는 장부를 씌워 트랜잭션을 가능케 하는 계약 계층. <strong>시간여행</strong> = 과거 버전 그대로 조회.</p>
</section>
...
<section class="slide">
  ...
  <h2>이것만은 가져가세요</h2>
  <ul>
    <li><strong>구조를 알아야 튜닝이 보인다</strong> — Driver·Executor·셔플이라는 실행 그림이 먼저.</li>
    <li><strong>추상화가 최적화를 부른다</strong> — 고수준 DataFrame이 대개 더 빠르다.</li>
    <li><strong>셔플이 늘 범인이다</strong> — 성능 튜닝은 셔플을 줄이거나 다스리는 일.</li>
    <li><strong>배치와 스트림은 한 엔진</strong> — 배치에서 익힌 게 실시간으로 이어진다.</li>
    <li><strong>Spark는 레이크하우스의 처리 엔진</strong> — 테이블 포맷과 만나 완성된다.</li>
  </ul>
</section>

<section class="slide slide--title">
  <p class="deck-kicker">7단계 완주 🎉</p>
  <h2>나누고 · 최적화하고 ·<br>다시 모은다</h2>
  <p class="deck-lead">이 뼈대만 손에 쥐면, Spark UI를 읽고 병목을 짚어내는 안목이 남는다.</p>
  <p class="deck-note">더 알아보기 · <a href="/2026/07/16/spark-architecture-driver-executor.html">1단계 아키텍처</a> · <a href="/2026/07/16/spark-shuffle-partitioning-tuning.html">4단계 튜닝</a> · <a href="/2026/07/16/spark-iceberg-delta-lakehouse.html">7단계 레이크하우스</a></p>
</section>
