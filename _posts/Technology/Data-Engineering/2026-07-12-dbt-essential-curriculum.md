...
- [x] **모델과 materialization**: `SELECT` = 모델, view/table/ephemeral/incremental 구체화 전략 — [[상세](/2026/07/14/dbt-models-ref-sources.html)]
- [x] **ref와 source**: `ref()`가 세우는 모델 간 의존성, `source()`로 원천 선언·freshness — [[상세](/2026/07/14/dbt-models-ref-sources.html)]
- [x] **DAG와 계층형 모델링**: staging → intermediate → marts, dbt가 빌드 순서를 정하는 원리 — [[상세](/2026/07/14/dbt-models-ref-sources.html)]
...
- [x] **schema test와 data test**: 내장 제네릭 테스트, 커스텀 SQL 테스트, 심각도(severity) 설정 — [[상세](/2026/07/14/dbt-tests-documentation.html)]
- [x] **문서화**: `description`, `docs` 블록, `dbt docs generate`로 만드는 데이터 카탈로그 — [[상세](/2026/07/14/dbt-tests-documentation.html)]
- [x] **lineage**: 자동 생성 계보 그래프로 영향 분석(impact analysis)과 디버깅 — [[상세](/2026/07/14/dbt-tests-documentation.html)]
...
- [x] **Jinja 기초**: 변수·`if`·`for` 블록, 컴파일 시점 실행 모델, `dbt compile`로 생성된 SQL 읽기 — [[상세](/2026/07/14/dbt-macros-jinja.html)]
- [x] **매크로**: 재사용 로직을 함수로, 인자와 반환, 프로젝트 전역에서 호출하기 — [[상세](/2026/07/14/dbt-macros-jinja.html)]
- [x] **매크로 패키지**: `dbt_utils` 등 커뮤니티 매크로 활용과 직접 만든 매크로의 조합 — [[상세](/2026/07/14/dbt-macros-jinja.html)]
...
- [x] **incremental 모델**: `is_incremental()`, `unique_key` 병합, `on_schema_change`, full-refresh — [[상세](/2026/07/14/dbt-incremental-snapshots.html)]
- [x] **incremental 전략**: append·merge·delete+insert, 웨어하우스별 차이와 선택 기준 — [[상세](/2026/07/14/dbt-incremental-snapshots.html)]
- [x] **snapshot과 SCD Type 2**: `timestamp`/`check` 전략으로 변화 이력 보존하기 — [[상세](/2026/07/14/dbt-incremental-snapshots.html)]
...
- [x] **dbt packages**: `packages.yml`, 버전 관리, 사내 공통 모듈을 패키지로 뽑기 — [[상세](/2026/07/14/dbt-packages-ci.html)]
- [x] **Slim CI와 state**: `state:modified` 선택, `--defer`, PR에서 바뀐 모델만 빌드/테스트 — [[상세](/2026/07/14/dbt-packages-ci.html)]
- [x] **배포 파이프라인**: CI/CD 통합, 스케줄 실행(오케스트레이터 연동), 환경 분리(dev/prod) — [[상세](/2026/07/14/dbt-packages-ci.html)]
...
- [x] **세만틱 레이어의 문제의식**: 지표 정의 파편화, 단일 정의 → 다중 소비의 가치 — [[상세](/2026/07/14/dbt-semantic-layer-metrics.html)]
- [x] **MetricFlow**: semantic model, measure/metric/dimension, entity와 조인 해석 — [[상세](/2026/07/14/dbt-semantic-layer-metrics.html)]
- [x] **지표 표준화**: BI·노트북에서 동일 정의 재사용, 거버넌스와 신뢰 — [[상세](/2026/07/14/dbt-semantic-layer-metrics.html)]
